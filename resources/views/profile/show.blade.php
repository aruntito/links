    ...
    <div class="w-full space-y-4 mb-8">
        ...
        @foreach($featuredLinks as $link)
            <a href="{{ route('links.redirect', $link->id) }}" 
               ...
               class="block w-full bg-gradient-to-br from-white/10 to-white/5 border border-white/10 hover:border-white/30 hover:shadow-[0_0_20px_rgba(255,255,255,0.1)] hover:-translate-y-0.5 transition-all duration-300 rounded-2xl py-4 px-6 shadow-lg group relative overflow-hidden">
                ...
                @if($link->description)
                    <div class="text-sm text-gray-400 text-center mt-1 relative z-10 font-medium">{{ $link->description }}</div>
                @endif
            </a>
        @endforeach
        ...
        @foreach($otherLinks as $link)
            <a href="{{ route('links.redirect', $link->id) }}" 
               target="_blank" rel="noopener noreferrer"
               class="block w-full bg-white/5 border border-white/10 hover:border-white/25 hover:bg-white/10 hover:-translate-y-0.5 transition-all duration-300 rounded-2xl py-4 px-6 shadow-md group">
                <div class="flex items-center justify-center">
                    <span class="font-medium text-gray-200 tracking-wide">{{ $link->title }}</span>
                </div>
            </a>
        @endforeach
        ...
        <div class="flex flex-wrap justify-center gap-4 mt-4">
            @foreach($socialLinks as $link)
                <a href="{{ route('links.redirect', $link->id) }}" 
                   target="_blank" rel="noopener noreferrer"
                   title="{{ $link->title }}"
                   class="flex items-center justify-center w-12 h-12 rounded-full bg-white/5 border border-white/10 hover:bg-white/20 hover:border-white/30 hover:scale-110 transition-all duration-300 text-gray-300 hover:text-white shadow-lg">
