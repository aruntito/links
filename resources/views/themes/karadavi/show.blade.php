...
@section('title', $profile->seo_title ?: $profile->name . ' | KARADAVI')
...
<div class="flex flex-col items-center w-full animate-fade-in-up font-sans">
    ...
    @if($profile->avatar)
        <img src="{{ Storage::disk('public')->url($profile->avatar) }}" 
             ...
             class="w-32 h-32 rounded-full object-cover mb-8 grayscale hover:grayscale-0 transition-all duration-700">
    @else
        <div class="w-32 h-32 rounded-full mb-8 bg-[#1a1a1a] flex items-center justify-center text-3xl font-light text-[#888888] tracking-widest border border-[#333333]">
            {{ strtoupper(substr($profile->name, 0, 2)) }}
        </div>
    @endif

    <!-- Name & Verified -->
    <h1 class="text-3xl font-light text-[#f3f3f3] flex items-center gap-2 mb-2 tracking-wide font-serif">
        {{ $profile->name }}
        @if($profile->is_verified)
            <svg class="w-5 h-5 text-[#888888]" fill="currentColor" viewBox="0 0 24 24">
    ...
    <!-- Headline -->
    @if($profile->headline)
        <p class="text-[#a1a1a1] font-normal text-sm mb-4 text-center tracking-wide">{{ $profile->headline }}</p>
    @endif

    <!-- Bio -->
    @if($profile->bio)
        <p class="text-[#888888] text-center text-sm mb-12 max-w-sm leading-relaxed font-serif italic">{{ $profile->bio }}</p>
    @endif
    ...
    <div class="w-full space-y-3 mb-12">
        ...
        @if($mainLinks->isEmpty() && $socialLinks->isEmpty())
            <div class="w-full py-12 text-center text-[#555555] text-sm font-light border-b border-[#333333]">
        ...
        @foreach($featuredLinks as $link)
            <a href="{{ route('links.redirect', $link->id) }}" 
               ...
               class="block w-full bg-[#161616] border border-[#333333] hover:border-[#666666] hover:bg-[#1a1a1a] transition-colors duration-500 py-5 px-6 group">
                <div class="flex items-center justify-between">
                    <span class="font-normal text-[#e5e5e5] tracking-wide text-lg">{{ $link->title }}</span>
                    <svg class="w-5 h-5 text-[#444444] group-hover:text-[#a1a1a1] transition-colors duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M17 8l4 4m0 0l-4 4m4-4H3"></path></svg>
                </div>
                @if($link->description)
                    <div class="text-sm text-[#777777] mt-2 font-light">{{ $link->description }}</div>
                @endif
            </a>
        @endforeach
        ...
        @foreach($otherLinks as $link)
            <a href="{{ route('links.redirect', $link->id) }}" 
               target="_blank" rel="noopener noreferrer"
               class="block w-full bg-transparent border-b border-[#222222] hover:border-[#555555] transition-colors duration-500 py-4 px-2 group">
                <div class="flex items-center justify-between">
                    <span class="font-light text-[#cccccc] tracking-wide">{{ $link->title }}</span>
                    <svg class="w-4 h-4 text-[#333333] group-hover:text-[#888888] transition-colors duration-500" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.5" d="M9 5l7 7-7 7"></path></svg>
                </div>
            </a>
        @endforeach
        ...
        <div class="flex flex-wrap justify-center gap-6 mt-4">
            @foreach($socialLinks as $link)
                <a href="{{ route('links.redirect', $link->id) }}" 
                   target="_blank" rel="noopener noreferrer"
                   title="{{ $link->title }}"
                   class="text-[#555555] hover:text-[#eeeeee] transition-colors duration-300">
